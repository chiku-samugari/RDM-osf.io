...
urlpatterns = [
    ...
    url(r'^statistical_status_default_storage/(?P<region_id>[0-9]+)/$', views.StatisticalStatusDefaultInstitutionalStorage.as_view(),
        name='statistical_status_default_storage'),
    url(r'^recalculate_quota/$', views.RecalculateQuota.as_view(), name='recalculate_quota'),
    url(r'^recalculate_quota_of_users_in_institution/(?P<region_id>[0-9]+)/$',
