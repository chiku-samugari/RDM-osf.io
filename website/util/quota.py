...
def update_used_quota(self, target, user, event_type, payload):
    ...
    if event_type not in [FileLog.FILE_MOVED, FileLog.FILE_COPIED, FileLog.FILE_RENAMED]:
        try:
            ...
            logger.warning('BaseFileNode not found, cannot update used quota!')
            return
    ...
    if isinstance(target, AbstractNode):
        ...
        if storage_type == ProjectStorageType.CUSTOM_STORAGE:
            ...
            if payload['destination']['provider'] == 'osfstorage':
                if payload['destination']['kind'] == 'file':
                    _des_path = payload['destination']['path']
                else:
        ...
        if institution is not None:
            ...
            if region is None:
                ...
                if user_settings.default_region._id != region._id:
                    ...
                    logger.info(u'user={}, institution={}, user_settings.set_region({})'.format(
                        user, institution.name, region.name))
...
def get_region_id_of_institutional_storage_by_path(
        target, provider, path, storage_type,
        file_node=None, root_path=None):
