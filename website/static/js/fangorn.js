...
function gotoFileEvent (item, toUrl) {
    ...
    if (ADDON_PROVIDER.includes(item.data.provider) &&
        item.data.root_path !== undefined &&
        item.data.root_path != null) {
        redir.segment('files')
            .segment(item.data.provider)
            .segment(item.data.root_path)
            .segmentCoded(item.data.path.substring(1));
    } else {
        redir.segment('files')
            .segment(item.data.provider)
            .segmentCoded(item.data.path.substring(1));
...
function setCurrentFileID(tree, nodeID, file) {
    ...
    if (file.provider === 'figshare') {
        ...
        for (var j = 0; j < tree.children.length; j++) {
            ...
            if (nodeID === child.data.nodeId &&
                child.data.provider === file.provider &&
                isSelectedItem) {
            ...
            if (item.data.provider === 'osfstorage' || ADDON_PROVIDER.includes(item.data.provider)) {
                ...
                if (quota.responseJSON || storage_quota.responseJSON) {
                    ...
                    if (quota.used + file.size > quota.max * window.contextVars.threshold ||
                        storage_quota.used + file.size > storage_quota.max * window.contextVars.threshold) {
                        $osf.growl(
                            gettext('Quota usage alert'),
                            sprintf(gettext('You have used more than %1$s% of your quota.'),
