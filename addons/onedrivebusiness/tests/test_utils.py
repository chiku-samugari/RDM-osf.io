...
from osf_tests.factories import ProjectFactory, AuthUserFactory, InstitutionFactory, RegionFactory
...
class TestOneDriveBusinessAddonTestCase(OAuthAddonTestCaseMixin, AddonTestCase):
    ...
    def test_get_region_external_account(self):
        ...
        with mock.patch('osf.models.rdm_addons.RdmAddonOption.objects.filter',
                        return_value=user.affiliated_institutions):
            with mock.patch('addons.osfstorage.models.Region.objects.filter',
                            return_value=user.affiliated_institutions):
                with mock.patch('osf.models.region_external_account.RegionExternalAccount.objects.filter',
                                region_filter):
    ...
    def test_get_region_external_account_with_region_not_exists(self):
        ...
        with mock.patch('addons.osfstorage.models.Region.objects.filter', return_value=user.affiliated_institutions):
            with mock.patch('osf.models.region_external_account.RegionExternalAccount.objects.get',
                            return_value=region):
                res = utils.get_region_external_account(node_setting)
                assert res is None
