...
def get_two_addon_options(institution_id, allowed_check=True):
    ...
    management_addon_options = get_rdm_addon_option(
        institution_id, DropboxBusinessManagementProvider.short_name,
        create=False)
    if (fileaccess_addon_options is None
            or management_addon_options is None
            or not fileaccess_addon_options.exists()):
        return None
...
def dict_folder_groups(client, team_folder_id):
    ...
    while has_more:
        ...
        for group in lst.groups:
            ...
            group_id_to_name[group_info.group_id] = group_info.group_name
